...
## Find original code on:
## https://github.com/TobiasRoeddiger/GazePointHeatMap

## This allows it to be run as a python method.
def load_mask(path):
    img = Image.open(path).convert('L')
    img = ~numpy.array(img,dtype=bool)
    return img
...
def draw_heatmap(gazepoints, dispsize, imagefile=None, maskfile=None, alpha=0.5, savefilename=None, gaussianwh=200, gaussiansd=None):
    ...
    for i in range(0, len(gazepoints)):
        ...
        if (not 0 < x < dispsize[0]) or (not 0 < y < dispsize[1]):
            ...
            try:
                heatmap[y:y + vadj[1], x:x + hadj[1]] += gaus[vadj[0]:vadj[1], hadj[0]:hadj[1]] * gazepoints[i][1]
            except:
    ...
    if maskfile is not None:
        img_mask =  load_mask(os.path.join(DATASET_DIR, "Masks", maskfile))
        ax.imshow(img_mask, alpha=0.2)
